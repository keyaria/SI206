...
def find_urls(s):
    ...
    lst = list()
    #other = re.findall('http[s]?://[a-zA-Z0-9]+([.]\S{2,})+',s)
...
def get_umsi_data():
    ...
    lasturl = "https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All&page=12"
    count = 13
    ...
    for ur in range(count):
        ...
        #Putting info into 2 lists
        lst = list()
        otherlst = list()

        #Finding the link on the page
        tags = soup('a', title="Go to next page")
        #If the tags are greater than 0 than it  changes the url for the code to work 
        if len(tags) > 0:
            prefix = "https://www.si.umich.edu"
            url = prefix + tags[0].get('href',None)

        #Finding title of person
       ...
       #Finding the name of the person 
        for name in soup.find_all(property="dc:title"):
            otherlst.append(name.text)

        #Looping to put both lists in to the dictionary. Name is key
        for i in range(len(otherlst)):

            umsi_titles[otherlst[i]] = lst[i]
...
def num_students(data):
    ...
    #Counting number of student based on key
    for key in data:
